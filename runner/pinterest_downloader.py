...
class PinterestVideoDownloader:
    ...
    def get_video_info(self, url):
        try:
            ...
            print("Successfully fetched pin data")
    ...
    def extract_pin_id(self, url):
        ...
        # Direct pin URL: https://www.pinterest.com/pin/123456789/
    ...
    def scrape_pin_page(self, pin_id):
        try:
                        ...
                        if 'resourceResponses' in data:
                            for resource in data['resourceResponses']:
                                if resource.get('resource', {}).get('name') == 'PinResource' and 'response' in resource and 'data' in resource['response']:
                                    print("Found pin data in PWS_DATA")
                                ...
                                if 'PinResource' in resource_key:
                                    for pin_id_key in data['resources'][resource_key]:
                                        if 'data' in data['resources'][resource_key][pin_id_key]:
                                            print("Found pin data in initial data")
            ...
            # Method 4: Look for video data in JSON-LD structured data
                ...
                try:
                    ...
                    if data.get('@type') == 'VideoObject':
                        ...
                        return {
                            ...
                            'videos': {
                                'video_list': {
                                    'V_720P': {'url': data.get('contentUrl', ''), 'duration': 0}
                                }
                            },
                            'images': {
                                '236x': {'url': data.get('thumbnailUrl', '')}
                            },
                            'title': data.get('name', 'Pinterest Video'),
    ...
    def extract_video_info(self, pin_data):
        ...
        if 'videos' not in pin_data:
            print("No videos found in pin data")
            return None
        ...
        if not video_data:
            print("No video data found in video_list")
            return None
